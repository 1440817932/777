      ...
      <el-table :data="goodsList" border stripe>
        ...
        <el-table-column prop="status" label="上架状态">
          <template slot-scope="scope">
            <el-switch :active-value="1" :inactive-value="0" v-model="scope.row.upStatus" @change="userStateChanged(scope.row)"></el-switch>
          </template>
        </el-table-column>
    ...
    >
      ...
      >
        ...
        <el-form-item label="物品价格" prop="price">
          <el-input v-model="editArticleForm.price"></el-input>
        </el-form-item>
        <el-form-item label="物品名称" prop="name">
          <el-input v-model="editArticleForm.name"></el-input>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    // 根据分页获取对应的商品列表
    async getGoodsList () {
      const { data: res } = await this.$http.post('article/list', this.queryInfo)
      if (res.code !== 0) {
      ...
      this.$refs.editArticleFormRef.validate(async valid => {
        ...
        const { data: res } = await this.$http.post(
          '/article/update',
          {
            ...
            price: this.editArticleForm.price,
            name: this.editArticleForm.name
          }
        )
        if (res.code !== 0) {
          this.$message.error('更新物品信息失败！')
        }
        // 隐藏添加用户对话框
        this.articleVisible = false
        this.$message.success('更新物品信息成功！')
        this.getGoodsList()
      })
    },
    // 编辑商品信息
    async showEditArticleDialog (id) {
      this.id.id = id
      const { data: res } = await this.$http.post('/article/info', this.id)
      if (res.code !== 0) {
    ...
    async userStateChanged (userInfo) {
      ...
      const { data: res } = await this.$http.post('/article/update', userInfo)
      if (res.code !== 0) {
        userInfo.upStatus = !userInfo.upStatus
    ...
    async removeById (id) {
      ...
      const { data: res } = await this.$http.post('/article/delete', this.id)
      if (res.code !== 0) {
